...
window.deleteTarget = async function(url) {
  ...
  if (!overlay) {
    ...
    overlay.style.cssText = 'position: fixed; top: 0; left: 0; width: 100vw; height: 100vh; background: rgba(0,0,0,0.5); z-index: 99999; display: flex; align-items: center; justify-content: center; backdrop-filter: blur(4px); transition: all 0.3s ease-in-out; opacity: 0; visibility: hidden;';
    overlay.innerHTML = `
      <div id="delete-modal" class="modal-content" style="max-width: 450px; width: 90%; background-color: #ffffff; padding: 30px 25px; border-radius: 12px; text-align: center; box-shadow: 0 10px 30px rgba(220,20,60,0.15); border-top: 4px solid var(--primary); transition: transform 0.3s ease-in-out; transform: scale(0.9);">
        <h2 style="color: var(--primary); margin-top: 0; display: flex; align-items: center; justify-content: center; gap: 10px; margin-bottom: 15px;"><i class="ph-fill ph-trash" style="font-size: 2rem;"></i> Xóa Mục Tiêu</h2>
        <p style="margin-bottom: 25px; line-height: 1.6; font-size: 1.05rem; color: var(--text-dark);">Bạn có chắc chắn muốn xóa mục tiêu này khỏi danh sách theo dõi không?</p>
        
        <div style="display: flex; gap: 12px;">
          <button id="btn-cancel-delete" class="btn" style="flex: 1; justify-content: center; background: #f1f5f9; color: var(--text-dark); border: 1px solid var(--border); padding: 12px; font-weight: bold; border-radius: 8px;">Hủy</button>
          <button id="btn-confirm-delete" class="btn btn-primary" style="flex: 1; justify-content: center; margin: 0; padding: 12px; font-weight: bold; font-size: 1.05rem; border-radius: 8px;">Xóa</button>
        </div>
      </div>
    `;
  ...
  const modalContent = document.getElementById('delete-modal');
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (successCondition) {
          ...
          const successTitle = tier === "degrade" ? "Đã hủy gói!" : "Thành công!";
          const successMessage = tier === "degrade" 
            ? `Tài khoản của bạn đã được hạ cấp về gói <b style="color: var(--primary); font-size: 1.1rem;">FREE</b>`
            : `Chúc mừng bạn đã nâng cấp thành công lên gói <b style="color: var(--primary); font-size: 1.1rem;">${data.tier.toUpperCase()}</b>`;
          ...
          successOverlay.innerHTML = `
            <div style="background: var(--surface); padding: 3rem 2rem; border-radius: 20px; text-align: center; width: 100%; max-width: 400px; box-shadow: 0 20px 40px rgba(0,0,0,0.4); transform: scale(0.9); animation: popIn 0.4s cubic-bezier(0.175, 0.885, 0.32, 1.275) forwards;">
              ...
              <h2 style="margin: 0 0 10px 0; color: var(--text-main); font-size: 1.8rem;">${successTitle}</h2>
              <p style="color: var(--text-muted); font-size: 1.05rem; margin-bottom: 2rem; line-height: 1.5;">${successMessage}</p>
              <button class="btn btn-primary" style="width: 100%; justify-content: center; padding: 14px; font-size: 1.1rem; border-radius: 12px; font-weight: bold; box-shadow: 0 8px 16px rgba(var(--primary-rgb), 0.3);" onclick="window.location.reload()">Đóng</button>
            </div>
            <style>
              @keyframes popIn {
...
window.showLimitToast = function(message) {
  ...
  if (!toastContainer) {
    ...
    toastContainer.style.cssText = 'position: fixed; top: 150px; right: 20px; z-index: 999999; display: flex; flex-direction: column; gap: 10px; align-items: flex-end; pointer-events: none;';
  ...
  toast.style.cssText = 'background: #ffffff; padding: 12px; border-radius: 10px; box-shadow: 0 8px 30px rgba(220,20,60,0.25); display: flex; flex-direction: column; transform: translateX(120%); opacity: 0; transition: all 0.4s cubic-bezier(0.175, 0.885, 0.32, 1.275); border-left: 4px solid var(--primary); font-family: "Inter", sans-serif; width: 220px; pointer-events: auto;';
  
  toast.innerHTML = `
    <div style="display: flex; gap: 8px; margin-bottom: 10px; align-items: flex-start;">
        <i class="ph-fill ph-warning-circle" style="color: var(--primary); font-size: 1.4rem; margin-top: 1px;"></i>
        <div style="flex: 1; font-size: 0.85rem; line-height: 1.35; color: var(--text-dark);">
            Đã đạt số lượng target tối đa, nếu có nhu cầu hãy nâng cấp thêm
        </div>
    </div>
    <button onclick="showUpgradeModal(); this.closest('#toast-container > div').remove();" class="btn btn-primary" style="width: 100%; padding: 8px; border-radius: 6px; font-weight: bold; font-size: 0.9rem; justify-content: center;">Nâng Cấp Ngay</button>
  `;
